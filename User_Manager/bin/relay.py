...
class RELAY():
    ...
    def processTestVector(self,msg):
        ...
        flag = int(msg["flag"])
        userIdx = int(msg["usrIdx"]) % (len(usernames) + 1)
        cmd = int(msg["cmd"])
        index = int(msg["index"]) % (len(usernames) + 1)
            ...
            if (userIdx == len(usernames)): #Pass invalid user
                ...
                if (flag == 1): #Pass invalid password
                    ...
                    if (cmd == 1): #Send message
                        if (index == len(usernames)): #Pass invalid username
                            msg = "{username}\n".format(username = username)
                            sys.stdout.write(msg)
                            sys.stdout.flush
                        else: #Pass valid username
                            msg = "{username}\n".format(username = usernames[index])
                            sys.stdout.write(msg)
                            sys.stdout.flush
                ...
                if (flag == 1): #Invalid admin login
                    ...
                    if (cmd == 1): #Logout
                        ...
                        if (index == len(usernames)): #Pass invalid user
                            msg = "{username}\n".format(username = username)
