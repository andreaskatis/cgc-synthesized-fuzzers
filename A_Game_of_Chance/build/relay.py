...
class RELAY():
    ...
    def processTestVector(self,msg):
        ...
        def tohex(val, nbits):
            return "\\x" + format(((val + (1 << nbits)) % (1 << nbits)), 'x')

        msgcmd = int(msg["command"])
        cmd = (["\x21\x00\x00\x00\x00\x00", "\x00\x5f\x00\x00\x00\x00", "\x00\x00\x3c\x00\x00\x00",
            "\x00\x00\x00\x42\x00\x00", "\x00\x00\x00\x00\x63\x00", "\x00\x00\x00\x00\x00\x51"][msgcmd % 6]) if (0 <= msgcmd <= 5) else str(msgcmd)
        size = int(msg["size"])
        deck = []
        ...
        hexSize = "" #Convert size to a hex representation in little endian
        hexSize += hexConv[size] if 0 <= size <= 100 else tohex(size, 64)
        hexSize += "\x00\x00\x00"

        for char in msg["deck"]:
            deck.append(int(char))
        hexDeck = ""
        for num in deck:
            hexDeck += "{val}".format(val = hexConv[num])
        snd = True if msg["snd"] == 1 else False

        msg = ""
        
        if cmd == "\x21\x00\x00\x00\x00\x00": #New deck
            msg += "{cmd}".format(cmd = cmd)
            msg += "{size}".format(size = hexSize)
            msg += "{deck}\n".format(deck = hexDeck[:size])
