...
class RELAY():
    ...
    def processTestVector(self,msg):
        ...
        cmd = (["\x21\x00\x00\x00\x00\x00", "\x00\x5f\x00\x00\x00\x00", "\x00\x00\x3c\x00\x00\x00",
            "\x00\x00\x00\x42\x00\x00", "\x00\x00\x00\x00\x63\x00", "\x00\x00\x00\x00\x00\x51"][msgcmd % 6])
            # if (0 <= msgcmd <= 5) else str(msgcmd)
        size = int(msg["size"])
        deck = []
        ...
        hexSize += hexConv[size]
        hexSize += "\x00\x00\x00"

        # if 0 <= size <= 100 else tohex(size, 64)
