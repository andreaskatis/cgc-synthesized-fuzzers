...
class RELAY():
    ...
    def processTestVector(self,msg):
        cmd = ["!aaaaa", "a_aaaa", "aa<aaa", "aaaBaa", "aaaaca", "aaaaa"][int(msg["command"]) % 6]
        size = int(msg["size"]) % 127
        deck = []
        ...
        hexSize += "{val}".format(val = hexConv[size])
        hexSize += "\x00\x00\x00"
        ...
        hexDeck = ""
        for num in deck:
            hexDeck += "{val}".format(val = hexConv[num + 2]) #Num received is [0, 8], but application expects [2, 10]

        msg = ""
        msg = "{cmd}".format(cmd = cmd)
        sys.stdout.write(bytes(msg, "utf-8").decode("utf-8"))
        sys.stdout.flush()

        if cmd == "!aaaaa": #New deck
            msg = "{size}".format(size = hexSize)
            sys.stdout.write(bytes(msg, "utf-8").decode("utf-8"))
            sys.stdout.flush()
            msg = "{deck}".format(deck = hexDeck[:size])
            sys.stdout.write(bytes(msg, "utf-8").decode("utf-8"))
            sys.stdout.flush()
